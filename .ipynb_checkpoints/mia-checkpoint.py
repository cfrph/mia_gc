...
<!DOCTYPE html>
<html>
<head><title>MultiLedger Import Tool</title></head>
...
def safe_process(handler, success_kwargs):
    try:
        results = handler()
        return render_template_string(SUCCESS_HTML, **success_kwargs(*results))
    except Exception as e:
        import traceback, logging
        logging.exception("Exception in processing upload")
        return f"<h3 style='color:red;'>ERROR: {type(e).__name__}</h3><pre>{str(e)}\n{traceback.format_exc()}</pre>", 500
...
def upload_tcb():
    def handler():
        ...
        return credit_csv, debit_csv, unmapped_csv
    def success_kwargs(credit_csv, debit_csv, unmapped_csv):
        return {
            "bank_name": "TCB",
            "import_file": os.path.basename(credit_csv),
            "unmapped_file": os.path.basename(unmapped_csv) if unmapped_csv else None
        }
    return safe_process(handler, success_kwargs)
...
def upload_capone():
    def handler():
        ...
        return import_csv, unmapped_csv
    def success_kwargs(import_csv, unmapped_csv):
        return {
            "bank_name": "CapOne",
