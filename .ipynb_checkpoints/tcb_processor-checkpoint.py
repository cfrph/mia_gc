...
BANK_PARSER_ID = os.environ.get("GCP_PROCESSOR_ID", "6df7b78a3654d182")
...
def extract_bank_statement_entities(pdf_path):
    ...
    name = client.processor_path(GCP_PROJECT_ID, GCP_LOCATION, BANK_PARSER_ID)
...
def process_tcb_statement(pdf_path, gj_startnum, dp_startnum, output_folder, timestamp):
    txns = extract_bank_statement_entities(pdf_path)
    df = pd.DataFrame(txns)
    # Always fill missing columns for robustness
